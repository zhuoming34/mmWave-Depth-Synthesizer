function signal_array = simulate_radar_signal_v2(reflector_cart_v)
% Synth Radar signal
...
% Output: "signal_array" FMCW beat signals in the antenna array
    ...
    for kp = 1:length(reflector_cart_v)
        ...
        path_loss = repmat(1/d_TX2reflector/d_RX2reflector,[length(t_ax),1,1]);
    ...
    for rng = 1:size(signal_array,1)
        signal_array_temp = reshape(signal_array(rng,:,:),[array_size(1),array_size(2)]);
        signal_array(rng,:,:) = awgn(signal_array_temp,-10); % introduce noise
    end
end