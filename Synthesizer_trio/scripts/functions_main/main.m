    ...
    for kso = start_idx:stop_idx
        ...
        for cs = 1:4
            if cs ~= 5
                ...
                topvw.lim = [min(cam_top);max(cam_top)]; % find the limits in all three dimensions 
                [tbbox_x, tbbox_y, tbbox_z] = meshgrid(topvw.lim(:,1),topvw.lim(:,2),topvw.lim(:,3)); % 8 vertices of the bounding box of the point cloud
