...
function main_solo(obj_name, CAD_idx, start_idx, stop_idx)
    ...
    obj_addr = strcat("..",SLASH,"results",SLASH, obj_name, num2str(CAD_idx));
    ...
    logging_solo(logFile, obj_name, CAD_idx, N_RX_az, N_RX_el, atn_bdl,...
        ...
        scene_lim_edge, translate_lim_1, threshold_factor)
    %type(logFile);
    
    %% load the surface model
    
    cart_v = load(strcat("..",SLASH,"CAD",SLASH,obj_name,"_",num2str(CAD_idx),".mat")).cart_v;
    ...
    cad_v = cad_v_struct;
    cad_v.CAD_idx = CAD_idx;
    cad_v.N_pt = length(cart_v);
    cad_v.cart_v = cart_v;
    cad_v.lim = [min(cart_v);max(cart_v)]; % find the limits in all three dimensions 
    [bbox_x, bbox_y, bbox_z] = meshgrid(cad_v.lim(:,1),cad_v.lim(:,2),cad_v.lim(:,3)); % 8 vertices of the bounding box of the point cloud
    cad_v.bbox = [bbox_x(:), bbox_y(:), bbox_z(:)]; 
    clear cart_v bbox N_pt cad_idx;
    cad1_v_origin = cad_v;
    ...
    for kso = start_idx:stop_idx
        ...
        for cs = 1:4                    
            if cs ~= 5
                view_idx = cs; 
                cad_scene_v = cad1_v_origin;
                %% Rotate
                if view_idx == 1
                    new_rotate = rotate_ang(randi(length(rotate_ang))); % randomly select a rotation angle and store it in the pc structure
                    cad_scene_v.rotate = new_rotate;
                     %cad_scene_v.rotate = mod(cad_scene_v.rotate*(randi(1)*2-1),180);
                else
                    cad_scene_v.rotate = new_rotate + sensor_ang_deg(view_idx);
                end

                % inline function for 2D rotation
                rotate2d =  @(x, M) (x(:, 1:2) * M);
                rotate_angle_rad = cad_scene_v.rotate/180*pi;
                ...
                cad_scene_v.cart_v(:,1:2) = rotate2d(cad_scene_v.cart_v, rotation_matrix); % rotate the point cloud 
                cad_scene_v.bbox(:,1:2) = rotate2d(cad_scene_v.bbox, rotation_matrix); % rotate the bounding box
                cad_scene_v.lim = [min(cad_scene_v.cart_v);max(cad_scene_v.cart_v)]; % update the limits in all three dimensions
                %% Translation
                translate_x_rng = (translate_lim_1(1,1)-cad_scene_v.lim(1,1)) : translate_x_res : (translate_lim_1(1,2)-cad_scene_v.lim(2,1)); % range of translation along x axis
                translate_y_rng = (translate_lim_1(2,1)-cad_scene_v.lim(1,2)) : translate_y_res : (translate_lim_1(2,2)-cad_scene_v.lim(2,2)); % range of translation along y axis
                %disp('trans lim')disp(translate_lim)disp('car scene lim') disp(cad_scene_v.lim)disp('trans x rng')disp(translate_x_rng)disp('trans y rng')disp(translate_y_rng)
                switch view_idx 
                    case 1
                ...
                % translate
                cad_scene_v.translate = [translate_x, translate_y, translate_z]; % store translation information in the pc structure
                cad_scene_v.cart_v = cad_scene_v.cart_v + cad_scene_v.translate; % translate the point cloud
                cad_scene_v.bbox = cad_scene_v.bbox + cad_scene_v.translate; % translate the bounding box
                cad_scene_v.lim = [min(cad_scene_v.cart_v);max(cad_scene_v.cart_v)]; % update the limits in all three dimensions

                if view_idx == 1
                    cam_top = cad_scene_v.cart_v;
                    %continue;
                end
                ...
                cad_scene_v.cart_v = cam_top;
                cad_scene_v.lim = [min(cad_scene_v.cart_v);max(cad_scene_v.cart_v)];
                [tbbox_x, tbbox_y, tbbox_z] = meshgrid(cad_scene_v.lim(:,1),cad_scene_v.lim(:,2),cad_scene_v.lim(:,3)); % 8 vertices of the bounding box of the point cloud
                cad_scene_v.bbox = [tbbox_x(:), tbbox_y(:), tbbox_z(:)]; 
            end

            % convert unit from mm to m
            cad_scene_v.cart_v = cad_scene_v.cart_v/1000; 
            cad_scene_v.bbox = cad_scene_v.bbox/1000; 
        
            %% display status
            %format shortg; clk = clock; disp(clk);
            disp(" ");disp(string(datetime('now','TimeZone','local','Format','yyyy-MM-dd HH:mm:ss z')));
            disp(strcat(obj_name,"<",num2str(CAD_idx),">, placement<",num2str(ks),">, view<",num2str(view_idx),">"));
            
            %% Modle camera point reflectors in the scene
            disp("Generating depth image")
            [visible_cart_v_dep] = remove_occlusion_v1(cad_scene_v,"cam",0); % remove occluded body of the car for dep image
            ...
            %[visible_cart_v] = remove_occlusion(cad_scene_v); % remove occluded body of the car
            [visible_cart_v_rad] = remove_occlusion_v1(cad_scene_v,"rad",0); 
            % reflector for radar signal
            try
                reflector_cart_v = model_point_reflector(visible_cart_v_rad,cad_scene_v.bbox); % model point reflectors that reflect back to the radar receiver
            catch
                disp('error'); %continue;
            end
