...
cd("F:\3_Education\UMASS\Courses\droneSLAM\mmWave\hawkeye synthesizer\Synthesizer_vers\duo\scripts")
obj1_name = "car"; obj2_name = "desk";
obj1_idx = 1; obj2_idx = 4;
str_idx = 1; stp_idx = 1;
plc_range = str_idx:stp_idx;
scene_lim = [-3, 3; -3, 3; -1, 1];
vis = "off";
saveimg = 1;
addr0 = strcat("..",SLASH,"results",SLASH,obj1_name,num2str(obj1_idx),'-',...
    obj2_name,num2str(obj2_idx));   
...
for vib = 0:1
    ...
    for plc_idx = plc_range
        ...
        ind_folder = num2str(ceil(plc_idx/500));
        addr1 = strcat(addr0,"_",vib_postfix,SLASH,ind_folder,SLASH);
        loadaddr = strcat(addr1,"cartHeat",SLASH);
        saveaddr = strcat(addr1,"combined",SLASH);
        ...
        disp(strcat("loaded placement ", num2str(plc_idx)))
        ...
        %% combine maps
        method_sel = 1;
        sz = 128; disp(strcat("Cube size = ", num2str(sz)))
        ...
        ftitle = strcat(obj1_name,num2str(obj1_idx),"-",obj2_name,num2str(obj2_idx),...
            " placement ", num2str(plc_idx));
        if vib == 0
        ...
        %f0p = f0.Position;
        %f0.Position = [f0p(1),f0p(2),f0p(3),f0p(4)*0.4];
        ...
        prefix = strcat(obj1_name,"_",num2str(obj1_idx),"_placement_", num2str(plc_idx),"_",vib_postfix,"_");

        %szt = strcat('(',num2str(sz),'x',num2str(sz),'x',num2str(sz),')');
        szt = strcat(num2str(sz),'^3');
        if saveimg == 1
            if vis == "off"
                set(f0, 'CreateFcn', 'set(gcbo,''Visible'',''on'')'); 
...
function f = show_heatmap2d_cart4(heatmap,scene_lim,sz,tlt,subtlt,vis)   
    ...
    screenSize = get(0, 'screensize');
    f.Position = [screenSize(3)/5, screenSize(4)/4, screenSize(3)*11/15, screenSize(3)*3/15];
    %fp = f.Position; disp(fp(3)); disp(fp(4))
    
    % Visulize the radar heatmap side view
    radar_heatmap_side = squeeze(max(heatmap,[],2));
    f1 = subplot(131);  imagesc(radar_heatmap_side.');
    %f1.Position(4) = f1.Position(4) * 0.33;
    %f1.Position(2) = f1.Position(2) + f1.Position(4)*4/12;
    t1 = title("Side View");
    %t1.Position(3) = 0.2;%t1.Position(2) + f1.Position(4)/3;
    ...
    %f2.Position(4) = f2.Position(4) * 0.33;
    %f2.Position(2) = f2.Position(2) + f2.Position(4)*4/12;
    t2 = title("Front View");
    %t2.Position(3) = 0.5;%t2.Position(2) + t2.Position(2)/5;
    %set(gca,'XDir','reverse');
    set(gca,'XDir','normal');set(gca,'YDir','normal');
